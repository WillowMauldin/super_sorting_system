...
public class Navigation extends SessionAdapter {
    ...
    public void navigateTo(int x, int y, int z, String dimension) throws IOException, InterruptedException, Exception {
	if (((int) Math.floor(this.x)) == x && ((int) Math.floor(this.y)) == y && ((int) Math.floor(this.z)) == z && dimension.equals(this.getOperatorDimension())) {
	    return;
	}

	PathfindingResponse resp = this.operator.findPath(this.agent, new Location(
	...
	this.y = y;
	this.z = z + 0.5;
	this.client.send(new ServerboundMovePlayerPosPacket(
	    true,
	    false,
	    this.x,
	    this.y,
